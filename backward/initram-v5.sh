...
DEPENDENCIES=(backward.py /home/gridsan/aplatham/Programs/gromacs-2018.4/bin/gmx_mpi)
...
B="python2.7 backward.py -f $INP -raw $RAW -o $GRO -kick $KICK -sol -p $TOP -po $OTP -n $NDX -from $CG -to $AA"
echo $B; $B || exit
...
define=-DUSE_MODIFIED_TIP3P_EPS -DFLEXIBLE
integrator=steep
nsteps=$EMSTEPS
emstep=0.1
pbc=xyz
...
# Set up for running
G="/home/gridsan/aplatham/Programs/gromacs-2018.4/bin/gmx_mpi grompp -f $mdp -c $GRO -n $NDX -p $OTP -o $BASE -maxwarn 2"
echo $G; $G || exit

# Run
M="mpirun -np 16 /home/gridsan/aplatham/Programs/gromacs-2018.4/bin/gmx_mpi mdrun -deffnm $BASE -v"
echo $M; $M 
...
cat << __MDP__ > $mdp
define                   = -DUSE_MODIFIED_TIP3P_EPS -DPOSRES -DPOSRES_FC_BB=400.0 -DPOSRES_FC_SC=40.0
integrator               = md
nsteps                   = $MDSTEPS
dt                       = $DELTA_T
pbc                      = xyz
...
__MDP__
  ...
  # Set up run
  G="/home/gridsan/aplatham/Programs/gromacs-2018.4/bin/gmx_mpi grompp -f $mdp -c $GRO -r $BW -p $OTP -o $BASE -maxwarn 2"
  echo $G; $G || exit
...
do
  ...
  m=${mdp%.mdp}
  tag=$((++i))-md-${m##*/}
  G="/home/gridsan/aplatham/Programs/gromacs-2018.4/bin/gmx_mpi grompp -f $mdp -c $GRO -r $BW -p $OTP -o $tag -maxwarn 2"
  echo $G; $G || exit
  M="mpirun -np 16 /home/gridsan/aplatham/Programs/gromacs-2018.4/bin/gmx_mpi mdrun -deffnm $tag -v"
  echo $M; $M 
  GRO=$tag.gro
